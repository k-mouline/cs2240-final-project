...
if [ "$#" -lt 4 ]; then
    echo "Usage: $0 <number_of_runs> <num_frames_list> <num_particles_list> <grid_size_list> <shape>"
    exit 1
fi
...
do
  ...
  echo "Run $((i+1))/$num_runs with frames=$num_frames, particles=$num_particles, grid_size=$grid_size", shape=$shape

  make clean all

  ./letitsnow $num_frames $num_particles $grid_size $shape
